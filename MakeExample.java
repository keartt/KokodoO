public class MakeExample {
	// 해당 코드는 프로젝트 대입 전 어떻게 알고리즘을 짜야지
	// 사용자의 선택에 맞는 최적의 동선을 제공할지를 알아보기 위해 코드를 작성했다.
	
	// 구글 지도 API 에서 데이터 요청 시 일본 지역 시간값 반환에 문제가 있어
	// 제공하는 지역들 사이의 이동시간 순번을 구하고 입력해 두었다. 
	
	// 본 사이트의 경우 A(음식) 카테고리 2개, B(장소) 카테고리 2~4개의 입력 값을 체크박스 형태로 받고 있다
	// 계산 방식은  A1과 A2 에서 선택된 B들 까지의 이동 시간 순번을 구하고, 2로 나누어 평균을 구한다 = b1,b2,b3,b4
	// 구해진 b1~4 들을 배열에 넣고 가장 작은 값과 두번째로 작은 값을 반환한다. = b1result, b2result
	// 첫번째 a > b1result > 두번째a > b2result 순으로 최종 반환한다.
